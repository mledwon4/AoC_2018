...
//Simple input for testing operation viability
var simpleInput = [
  "+1",
  "-2",
  "+3",
...
/// Obtain the signed Int value for a string input
///
/// - Parameter input: expected string input format is "+123", supporting either +/- in the first character position and any number of integer values following the first character
/// - Returns: A positive or negative integer value
func valueForInputString(input : String) -> Int {
  
  var inputString : String! = input
  
  let sign = inputString.removeFirst()
  var value : Int! = Int(inputString)
...
// ********** ********** ********** ********** **********
// Execute the Frequency checks below


//Init the frequency to 0
var frequency : Int = 0

//Create an array to save all of our experienced frequencies in
//var foundFrequencyArray : Array<Int> = []
var foundFrequencyDict : [String : Int] = [:]

//Create a flag to track if we've found a repeat
var repeatFrequencyFound = false

//Create a variable to track what iteration of our loop we're in
var iterationLoop = 0

while repeatFrequencyFound == false {
  
  iterationLoop += 1
  
  //Iterate through our frequency adjustments and apply them to our frequency
  for inputString in inputArray {
    let frequencyChange = valueForInputString(input: inputString)
    
    //print("Currently freq = \(frequency), change of \(inputString)")
    frequency += frequencyChange
    print("Result = \(frequency)")
    
    
    if foundFrequencyDict[String(frequency)] != nil {
      print("FOUND REPEAT! \(frequency)")
      repeatFrequencyFound = true
      break
    }
    
    let freqKey = String(frequency)
    foundFrequencyDict.updateValue(1, forKey: freqKey)
    //foundFrequencyArray.append(frequency)
  }
  
  print("--------------------------------------------------")
  print("For loop #\(iterationLoop)")
  print("Resulting Frequency = \(frequency)")
  print("Number of frequencies observed = \(foundFrequencyDict.keys.count)")
  print("--------------------------------------------------")
}
